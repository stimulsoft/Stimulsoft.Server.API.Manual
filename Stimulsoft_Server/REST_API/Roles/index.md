...
| **Name** | **Description** |
| --- | --- |
| [GET List](GET_List.md) | Getting a list of roles. |
| [GET Info](GET_Info.md) | Getting information about the role in a workspace of the logged-in user. |
| [POST Create](POST_Create.md) | Creating a new role in a workspace of the logged-in user. This command ignores the initialization key of role (field Key) and puts this value automatically. |
| [PUT Edit](PUT_Edit.md) | Changing roles of users in a workspace of the logged-in user. This command does not allow a change of the unique role name (field Key). |
| [DELETE](DELETE.md) | Removing a role from the current workspace. Removing a last role with administrator privileges isn't allowed. |
